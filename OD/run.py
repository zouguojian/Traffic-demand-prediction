...
from comparison_model.rnn import rnn
from comparison_model.lstm import lstm
from comparison_model.gru import gru
from comparison_model.gcn_lstm import gcn_lstm
from comparison_model.cnn_lstm import cnn_lstm
from comparison_model.mlrnn import mlrnn
from comparison_model.multi_convlstm import mul_convlstm
...
class Model(object):
    ...
    def model(self):
        ...
        '''
        normal=normalization.Normalization(inputs=self.placeholders['features'],out_size=self.para.features,is_training=self.placeholders['is_training'])
        normal.normal()
        ...
        if self.para.model_name=='gcn':
            ...
            for site_id in range(inputs.shape[2]):
                (c_states_, h_states_)=encoder_init.encoding(inputs[:,:,site_id,:])
                h_states_=tf.transpose(tf.convert_to_tensor(h_states_,dtype=tf.float32),[1,0,2]) #[batch, time ,h]
                h_states.append(tf.expand_dims(h_states_,axis=0))
            ...
            x_input = tf.reshape(x_input, shape=[-1, self.para.input_length, self.para.site_num, self.para.features])
            for site_id in range(x_input.shape[2]):
                h_states=r.encoding(x_input[:,:,site_id,:])
                pre = r.decoding(h_states)
                pres.append(tf.expand_dims(pre, axis=0))
            self.pres = tf.transpose(tf.concat(pres, axis=0), perm=[1, 0, 2])
            self.cross_entropy = tf.reduce_mean(
                tf.sqrt(tf.reduce_mean(tf.square(self.placeholders['labels'] - self.pres), axis=0)))

        elif self.para.model_name == 'gru':
            r = gru(batch_size=self.para.batch_size,
                    predict_time=self.para.output_length,
                    layer_num=self.para.hidden_layer,
                    nodes=self.para.hidden_size,
                    placeholders=self.placeholders)
            ...
            l = lstm(batch_size=self.para.batch_size,
                     predict_time=self.para.output_length,
                     layer_num=self.para.hidden_layer,
                     nodes=self.para.hidden_size,
                     placeholders=self.placeholders)
            ...
            x_input = tf.reshape(x_input, shape=[-1, self.para.input_length, self.para.site_num, self.para.features])
            for site_id in range(x_input.shape[2]):
                h_states=l.encoding(x_input[:,:,site_id,:])
                pre = l.decoding(h_states)
                pres.append(tf.expand_dims(pre, axis=0))
            self.pres = tf.transpose(tf.concat(pres, axis=0), perm=[1, 0, 2])
            self.cross_entropy = tf.reduce_mean(
                tf.sqrt(tf.reduce_mean(tf.square(self.placeholders['labels'] - self.pres), axis=0)))

        elif self.para.model_name=='gcn_lstm':
            '''
            return, the gcn output --- for example, inputs.shape is :  (32, 3, 162, 32)
            axis=0: bath size
            axis=1: input data time size
            ...
            l = gcn_lstm(batch_size=self.para.batch_size,
                         predict_time=self.para.output_length,
                         layer_num=self.para.hidden_layer,
                         nodes=self.para.hidden_size,
                         placeholders=self.placeholders)
            ...
            pres = []
            # x_input = self.placeholders['features']
            # inputs = tf.reshape(x_input, shape=[-1, self.para.input_length, self.para.site_num, self.para.features])
            for site_id in range(inputs.shape[2]):
                h_states=l.encoding(inputs[:,:,site_id,:])
                pre = l.decoding(h_states)
                pres.append(tf.expand_dims(pre, axis=0))
            self.pres = tf.transpose(tf.concat(pres, axis=0), perm=[1, 0, 2])
            self.cross_entropy = tf.reduce_mean(
        ...
        elif self.para.model_name=='gcn_rnn':
            '''
            return, the gcn output --- for example, inputs.shape is :  (32, 3, 162, 32)
            axis=0: bath size
            axis=1: input data time size
            ...
            l = rnn(batch_size=self.para.batch_size,
                     predict_time=self.para.output_length,
                     layer_num=self.para.hidden_layer,
                     nodes=self.para.hidden_size,
                     placeholders=self.placeholders)
        ...
        elif self.para.model_name=='rnn_gcn':
            ...
            for site_id in range(inputs.shape[2]):
                h_states=l.encoding(inputs[:,:,site_id,:])
                # pre = l.decoding(h_states)
                outputs.append(tf.expand_dims(h_states[:,-1,:], axis=0))
            outputs = tf.transpose(tf.concat(outputs, axis=0), perm=[1, 0, 2])

            '''
            return, the gcn output --- for example, inputs.shape is :  (32, 162, 1)
            axis=0: bath size
            axis=1: numbers of the nodes
            axis=2: output feature size
            '''
            model_gcn = self.model_func(self.placeholders, input_dim=128, para=self.para)
            model_gcn_ = []
            for time in range(self.para.batch_size):
                model_gcn_.append(tf.expand_dims(model_gcn.predict(outputs[time,:,:]), axis=0))
            output = tf.concat(values=model_gcn_, axis=0)
            pres = tf.reshape(output, shape=[self.para.batch_size,
                                               self.para.site_num,
                                               self.para.gcn_output_size])

            self.pres=tf.layers.dense(pres,units=self.para.output_length,reuse=tf.AUTO_REUSE)

            print('output shape is : ', output.shape)
            self.cross_entropy = tf.reduce_mean(
                tf.sqrt(tf.reduce_mean(tf.square(self.placeholders['labels'] - self.pres), axis=0)))

        elif self.para.model_name=='gcn_gru':
            '''
            return, the gcn output --- for example, inputs.shape is :  (32, 3, 162, 32)
            axis=0: bath size
            axis=1: input data time size
            ...
            l = gru(batch_size=self.para.batch_size,
                         predict_time=self.para.output_length,
                         layer_num=self.para.hidden_layer,
                         nodes=self.para.hidden_size,
                         placeholders=self.placeholders)
        ...
        elif self.para.model_name=='cnn_lstm':
            ...
            '''
            cnn_out=tf.concat([tf.expand_dims(l.cnn(tf.expand_dims(inputs[:,i,:,:],axis=3)),axis=1) for i in range(self.para.input_length)],axis=1)
            pos_es=l.position_em()
            pres = []
            for site_id in range(cnn_out.shape[2]):
                h_states=l.encoding(cnn_out[:, :, site_id, :])
                new_input = tf.concat([h_states[:, -1, :], pos_es[:, site_id, :]], axis=-1)
                new_input = tf.layers.dense(inputs=new_input, units=self.para.hidden_size)

                pre = l.decoding(new_input)
                pres.append(tf.expand_dims(pre, axis=0))
            self.pres = tf.transpose(tf.concat(pres, axis=0), perm=[1, 0, 2])
            self.cross_entropy = tf.reduce_mean(
                tf.sqrt(tf.reduce_mean(tf.square(self.placeholders['labels'] - self.pres), axis=0)))

        elif self.para.model_name == 'mlrnn':
            l = mlrnn(batch_size=self.para.batch_size,
                     predict_time=self.para.output_length,
                     layer_num=self.para.hidden_layer,
                     nodes=32,
                     placeholders=self.placeholders)
            ...
            ''' the output of multi_encoding shape is : [batch size, time size, site num, hidden size]'''
            mut_lstm=l.multi_encoding(x_input)
            print('mut_lstm shape is : ',mut_lstm.shape)
            pres1 = []
            for site_id in range(mut_lstm.shape[2]):
                pre = l.mul_decoding(mut_lstm[:,:,site_id,:],site_id)
                pres1.append(tf.expand_dims(pre, axis=0))
            self.pres1 = tf.transpose(tf.concat(pres1, axis=0), perm=[1, 0, 2])
            print('pres1 shape is : ',self.pres1.shape)

            pres2 = []
            for site_id in range(x_input.shape[2]):
                h_states=l.global_encoding(x_input[:,:,site_id,:])
                pre = l.glo_decoding(h_states)
                pres2.append(tf.expand_dims(pre, axis=0))
            self.pres2 = tf.transpose(tf.concat(pres2, axis=0), perm=[1, 0, 2])
            print('pres2 shape is : ', self.pres2.shape)


            self.pres=self.pres1*0.5 + self.pres2*0.5

            self.cross_entropy1 = tf.reduce_mean(
                tf.sqrt(tf.reduce_mean(tf.square(self.placeholders['labels'] - self.pres1), axis=0)))
            self.cross_entropy2 = tf.reduce_mean(
                tf.sqrt(tf.reduce_mean(tf.square(self.placeholders['labels'] - self.pres2), axis=0)))
            self.cross_entropy3=tf.reduce_mean(
                tf.sqrt(tf.reduce_mean(tf.square(self.pres2 - self.pres1), axis=0)))
            self.cross_entropy=self.cross_entropy1+self.cross_entropy2+self.cross_entropy3

        elif self.para.model_name == 'multiconvlstm':
            ...
            x_input = self.placeholders['features']
            x_input = tf.reshape(x_input, shape=[-1, self.para.input_length, self.para.site_num, self.para.features])
            h_states=mul_convl.encoding(x_input)
    ...
    def evaluate(self):
        ...
        np.savetxt('results1/results_label.txt',label_list,'%.3f')
        np.savetxt('results1/results_predict.txt', predict_list, '%.3f')
