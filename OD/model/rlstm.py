...
class rlstm(object):
    ...
    def t_state(self, layer):
        '''
        it divided train or test
        :return:
        '''
        with tf.variable_scope(name_or_scope='train_state_'+str(layer), reuse=tf.AUTO_REUSE):
            c_state=tf.Variable(tf.zeros(shape=[self.batch_size,self.nodes],dtype=tf.float32),name='c_state')
            h_state=tf.Variable(tf.zeros(shape=[self.batch_size,self.nodes],dtype=tf.float32),name='h_state')
    ...
    def calculate(self,input):
        ...
        for layer in range(self.layer_num):
            with tf.variable_scope(name_or_scope=str(layer)):
                c_state,h_state=self.t_state(layer)
